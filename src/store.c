...
static void caixa_atribuir_operador(Supermercado *sm, int idx) {
    ...
    strncpy(sm->caixas[idx].ultimoOperador, sm->caixas[idx].operador, MAX_OPERADOR - 1);
    sm->caixas[idx].ultimoOperador[MAX_OPERADOR - 1] = '\0';
}
...
static int redistribuir_fila(Supermercado *sm, HashClientes *hash, int idCaixa) {
...
int guardar_snapshot(const char *filename, const Supermercado *sm, const RegistoClientes *registo) {
    ...
    fprintf(f, "TOTAL_PRODUTOS_OFERECIDOS %d\n", sm->totalProdutosOferecidos);
    fprintf(f, "TOTAL_VALOR_OFERECIDO %.2f\n", sm->totalValorOferecido);
    fprintf(f, "EM_LOJA %d\n", sm->nClientesEmLoja);
    ...
    for (i = 0; i < sm->cfg.nCaixas; i++) {
        ...
        fprintf(f, "ULTIMO_OPERADOR %d %s\n", i + 1,
                cx->ultimoOperador[0] != '\0' ? cx->ultimoOperador : "-");
...
static void recomputar_ofertas_cliente(Cliente *cliente) {
    int i;
    ...
    Produto *prods, int nProds)
{
    ...
    recomputar_ofertas_cliente(cliente);
...
static int carregar_formato_novo(FILE *f, Supermercado *sm, HashClientes *hash, char *primeira_linha) {
    ...
    while (fgets(linha, sizeof(linha), f)) {
        ...
        } else if (strncmp(p, "TOTAL_PRODUTOS_OFERECIDOS", 25) == 0) {
            sscanf(p, "TOTAL_PRODUTOS_OFERECIDOS %d", &sm->totalProdutosOferecidos);
        } else if (strncmp(p, "TOTAL_VALOR_OFERECIDO", 21) == 0) {
            sscanf(p, "TOTAL_VALOR_OFERECIDO %f", &sm->totalValorOferecido);
        } else if (strncmp(p, "ULTIMO_OPERADOR", 15) == 0) {
            int num = 0;
            char nome[MAX_OPERADOR];
            if (sscanf(p, "ULTIMO_OPERADOR %d %63[^\n]", &num, nome) == 2) {
                int idx = num - 1;
                if (idx >= 0 && idx < sm->cfg.nCaixas && strcmp(nome, "-") != 0) {
                    strncpy(sm->caixas[idx].ultimoOperador, nome, MAX_OPERADOR - 1);
                    sm->caixas[idx].ultimoOperador[MAX_OPERADOR - 1] = '\0';
                }
            }
...
int inserir_novo_cliente(Supermercado *sm, HashClientes *hash, const char *idOriginal, const char *nome, Produto *produtos, int nProdutos) {
    ...
    id[sizeof(id) - 1] = '\0';
    trim(id);
    if (!id_valido(id) || !produtos || nProdutos <= 0) return INSERIR_CLIENTE_INVALIDO;
    ...
    for (p = 0; p < passos; ++p) {
        ...
        while (j < sm->nClientesEmLoja) {
            ...
            if (sm->instanteAtual >= c->instanteEntradaLoja + c->tempoCompraTotal) {
                ...
                if (destino >= 0 && fila_inserir(&sm->caixas[destino].fila, c)) {
        ...
        for (i = 0; i < sm->cfg.nCaixas; ++i) {
            ...
            if (c && !c->oferecimentoFeito && c->instanteEntradaFila >= 0) {
                int espera = (c->instanteInicioAtendimento >= 0)
                             ? c->instanteInicioAtendimento - c->instanteEntradaFila
                             : sm->instanteAtual - c->instanteEntradaFila;
...
int abrir_caixa_manual(Supermercado *sm, int idCaixa) {
    ...
    if (sm->caixas[idCaixa].estado == CAIXA_ABERTA) return 0;
    if (sm->caixas[idCaixa].estado == CAIXA_A_FECHAR) {
        printf("Caixa seleccionada encontra-se a encerrar, escolha uma caixa fechada para abrir.\n");
        return 0;
    }
...
int fechar_caixa_suave_manual(Supermercado *sm, int idCaixa) {
    ...
    if (sm->caixas[idCaixa].fila.tamanho == 0 && sm->caixas[idCaixa].emAtendimento == NULL) {
        sm->caixas[idCaixa].estado = CAIXA_FECHADA;
        sm->caixas[idCaixa].operador[0] = '\0';
        snprintf(detalhes, sizeof(detalhes), "Caixa %d fechada imediatamente (ja estava vazia)", idCaixa + 1);
        log_acao(sm->logFile, "FECHAR_CAIXA_IMEDIATA_MANUAL", detalhes);
    }
    return 1;
}

static int contar_outras_caixas_abertas(const Supermercado *sm, int ignorar) {
    int i, total = 0;
    for (i = 0; i < sm->cfg.nCaixas; i++) {
        if (i != ignorar && sm->caixas[i].estado == CAIXA_ABERTA) total++;
    }
    return total;
}
...
int fechar_caixa_imediata_manual(Supermercado *sm, HashClientes *hash, int idCaixa) {
    char detalhes[64];
    if (idCaixa < 0 || idCaixa >= sm->cfg.nCaixas) return 0;
    if (contar_outras_caixas_abertas(sm, idCaixa) <= 0) return 0;
    if (sm->caixas[idCaixa].emAtendimento) {
        ...
        if (destino >= 0 && fila_inserir(&sm->caixas[destino].fila, cliente)) {
            ...
            cliente->estavaEmAtendimento = estavaAntigo;
            cliente->instanteInicioAtendimento = inicioAntigo;
            sm->caixas[idCaixa].emAtendimento = cliente;
            sm->caixas[idCaixa].tempoRestanteAtendimento = tempoRestanteAntigo;
            return 0;
        }
    }
    if (!redistribuir_fila(sm, hash, idCaixa)) return 0;
    sm->caixas[idCaixa].estado = CAIXA_FECHADA;
    sm->caixas[idCaixa].operador[0] = '\0';
